...
{
	int index = rand() % 1000;
	Traveller* trvl = m_pplManager.GetTraveller(index);
	m_airport.LineUp(trvl);
	m_ptrTravellers.PushBack(trvl);
	trvl->state = LINEUP;
	DisplayMessage(trvl, LINEUP);
...
{
	Traveller* trvl = m_airport.NextInLine();
	m_airplane.Board(trvl);
	trvl->state = BOARDING;
	DisplayMessage(trvl, BOARDING);
...
{
	Traveller* trvl = m_airplane.Disembark();
	trvl->state = FINISHED;
	DisplayMessage(trvl, DISEMBARKING);
